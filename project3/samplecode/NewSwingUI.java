...
public class NewSwingUI implements UIContext {
  ...
  public void drawCircle(Point center, int radius) {
    if (center != null && radius > 0) {
      int x = Math.round((float)(center.getX() - radius));
      int y = Math.round((float)(center.getY() - radius));
      int diameter = radius * 2;
      // Draw a thick circle for better visibility
      graphics.drawOval(x, y, diameter, diameter);
      graphics.drawOval(x+1, y+1, diameter-2, diameter-2);
      graphics.drawOval(x-1, y-1, diameter+2, diameter+2);
    }
  }
